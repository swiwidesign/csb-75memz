...
containers.forEach((container) => {
  let cursors = gsap.utils.toArray(container.querySelectorAll(".cursor-muva"));

  if (cursors.length === 2) {
    cursors.forEach((cursor, index) => {
      let rotate = cursor.querySelector(".cursor-muva-image");
      let ease = "ease";
      ...
      container.addEventListener("mousemove", (e) => {
        let yDif = e.clientY - y, xDif = e.clientX - x;
        xTo(e.clientX);
        yTo(e.clientY);
